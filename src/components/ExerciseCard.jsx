...
export default function ExerciseCard(props) {
  ...
  return (
    <div className='p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>Exercise
        <div className='flex flex-col sm:flex-row sm:items-center'>
            <h4 className='text-3xl hidden sm:inline sm:text-4xl md:text-5xl font-semibold text-slate-400'> 0{i+1} </h4>
            <h2 className='capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-xl md:text-2xl flex-1 md:text-center sm:text-center'>{exercise.name.replaceAll("_", " ")}</h2>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4'>
            {['reps', 'rest', 'tempo'].map(info => {
                return (
                    <div key={info} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900'>
